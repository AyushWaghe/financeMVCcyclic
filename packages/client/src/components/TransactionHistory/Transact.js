...
function Transact() {
  ...
  const fetchTransactions = async () => {
    ...
    try {
      if (hasApplied) {
        // console.log("Applied");
        const response = await axios.get(`/fetchTransactions/fetch?userName=${userName}&month=${appliedMonth}`);
        setTransactions(response.data.transactions);
        setMonthTotal(response.data.monthTotal);
        console.log("Transactions:", response.data.transactions);
      } else {
        // console.log("Not appleid");
        const response = await axios.get(`/fetchTransactions/fetch?userName=${userName}&month=${Month}`);
        setTransactions(response.data.transactions);
        setMonthTotal(response.data.monthTotal);
        console.log("Transactions:", response.data.transactions);
      }
  ...
  const handleSubmit = async (e) => {
    ...
    if (isUpdating) {
      ...
      try {
        await axios.post(`/getTransactionAndStoreThem/updateTransaction`, null, {
          params: {
            ...transactionData,
      ...
      try {
        await axios.post('/getTransactionAndStoreThem/saveTransaction', transactionData);
      } catch (error) {
